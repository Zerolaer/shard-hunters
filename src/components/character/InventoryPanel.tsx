...
import { Backpack, CheckSquare, Coins, Recycle, Square } from "lucide-react";
...
export function InventoryPanel() {
  ...
  const setBulkSelected = useUiStore((s) => s.setBulkSelected);
  ...
  const visibleIds = visibleInventoryIds(inventory, filterSlot, filterRarity);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-2">
      <div className="es-plate flex shrink-0 flex-col gap-2 overflow-visible px-2.5 py-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="ml-auto inline-flex shrink-0 items-center gap-1 whitespace-nowrap text-[10px] text-[#8aa0b4]">
        ...
        {selectionMode && (
          <div className="flex flex-wrap items-center gap-1.5 border-t border-white/8 pt-2">
            ...
            <button type="button" onClick={() => setBulkSelected(visibleIds)} className="es-btn es-inv-control px-2.5">
              Видимые
            </button>
            <button type="button" onClick={clearBulkSelection} className="es-btn es-inv-control px-2.5">
              Сброс
            </button>
            <button
              type="button"
      ...
      {inventoryMessage && <p className="shrink-0 text-[11px] leading-tight text-amber">{inventoryMessage}</p>}

      <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 overflow-visible md:grid-cols-[minmax(0,1fr)_minmax(300px,40%)]">
        ...
        <aside className="min-h-0 min-w-0 overflow-y-auto md:sticky md:top-0">
          <CraftPanel />
        </aside>
      </div>
    </div>
